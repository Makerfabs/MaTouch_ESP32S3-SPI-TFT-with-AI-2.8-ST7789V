...
#include "voice/voice_auto_chat.h"
...
static void outbound_dispatch_task(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            esp_err_t voice_err = voice_bridge_speak_text(msg.content);
            if (voice_err != ESP_OK)
            {
                ESP_LOGW(TAG, "Voice speak failed for %s: %s (%s)", msg.chat_id, esp_err_to_name(voice_err), voice_bridge_get_unavailable_reason());
            }
            voice_auto_chat_notify_cycle_done();
        }
        else
        {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(asr_client_init());
    ESP_ERROR_CHECK(voice_bridge_init());
    ESP_ERROR_CHECK(voice_auto_chat_init());
...

    {
        ...
        ESP_ERROR_CHECK(voice_auto_chat_start());
        cron_service_start();
        heartbeat_start();
