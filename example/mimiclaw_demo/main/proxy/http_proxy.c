...
/* Temporarily raise proxy logs for TLS heap diagnostics */
...
proxy_conn_t *proxy_conn_open(const char *host, int port, int timeout_ms)
{
    ...
    log_heap_state("proxy_conn_open: before esp_tls_init");

    /* ── TLS handshake via esp_tls over tunnel ───────────────── */
    conn->tls = esp_tls_init();
    log_heap_state("proxy_conn_open: after esp_tls_init");
    if (!conn->tls) {
    ...
    log_heap_state("proxy_conn_open: before esp_tls_conn_new_sync");
    int ret = esp_tls_conn_new_sync(host, strlen(host), port, &cfg, conn->tls);
    log_heap_state("proxy_conn_open: after esp_tls_conn_new_sync");
    if (ret <= 0) {
...
{
    log_heap_state("proxy_conn_write: before write loop");
    int written = 0;
    while (written < len) {
        log_heap_state("proxy_conn_write: before esp_tls_conn_write");
        ssize_t ret = esp_tls_conn_write(conn->tls, data + written, len - written);
        log_heap_state("proxy_conn_write: after esp_tls_conn_write");
        if (ret > 0) {
            written += (int)ret;
