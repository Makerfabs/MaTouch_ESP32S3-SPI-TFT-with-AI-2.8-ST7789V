...
esp_err_t session_append_message(const char *chat_id, const cJSON *message)
{
    if (!chat_id || !message || !cJSON_IsObject(message)) {
    ...
    cJSON *obj = cJSON_Duplicate(message, 1);
    if (!obj) {
        fclose(f);
        return ESP_ERR_NO_MEM;
    }
    cJSON_DeleteItemFromObject(obj, "ts");
...
esp_err_t session_append(const char *chat_id, const char *role, const char *content)
{
    ...
    esp_err_t err = session_append_message(chat_id, obj);
    cJSON_Delete(obj);
    return err;
}
