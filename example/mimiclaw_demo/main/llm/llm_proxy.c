...
static void resp_buf_decode_chunked(resp_buf_t *rb)
{
    ...
    size_t i = 0;
    while (i < rb->len && (rb->data[i] == ' ' || rb->data[i] == '\t'))
        i++;
    if (i < rb->len && (rb->data[i] == '{' || rb->data[i] == '['))
        return;
...
static esp_err_t llm_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    if (provider_is_openai() || provider_is_volcengine()) // ✅ Add volcengine
    {
        if (s_api_key[0])
        {
                         ...
                         llm_response_t *resp)
{
    ...
    log_heap_state("llm_chat_tools: before create body");
    cJSON *body = cJSON_CreateObject();
    log_heap_state("llm_chat_tools: after create body");
    ...
    else
    {
        ...
        log_heap_state("llm_chat_tools: before duplicate messages");
        cJSON *msgs_copy = cJSON_Duplicate(messages, 1);
        log_heap_state("llm_chat_tools: after duplicate messages");
    ...
    log_heap_state("llm_chat_tools: before print body");
    char *post_data_tmp = cJSON_PrintUnformatted(body);
    log_heap_state("llm_chat_tools: after print body");
    cJSON_Delete(body);
    if (!post_data_tmp)
        return ESP_ERR_NO_MEM;

    size_t post_len = strlen(post_data_tmp);
    char *post_data = heap_caps_malloc(post_len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!post_data)
    {
        free(post_data_tmp);
        return ESP_ERR_NO_MEM;
    }
    memcpy(post_data, post_data_tmp, post_len + 1);
    free(post_data_tmp);
    log_heap_state("llm_chat_tools: after move post body to spiram");

    ESP_LOGI(TAG, "Calling LLM API with tools (provider: %s, model: %s, body: %d bytes)",
             s_provider, s_model, (int)post_len);
    ...
    int status = 0;
    log_heap_state("llm_chat_tools: before llm_http_call");
    esp_err_t err = llm_http_call(post_data, &rb, &status);
    log_heap_state("llm_chat_tools: after llm_http_call");
    free(post_data);
