...
static void agent_loop_task(void *arg)
{
    ...
    while (1) {
        ...
        //     log_heap_state("agent_loop: before voice runtime image release");
        //     esp_err_t release_err = bsp_display_release_runtime_image();
        //     if (release_err == ESP_OK) {
        //         log_heap_state("agent_loop: after voice runtime image release");
        //     }
        // }
        ...
        int history_limit = (strcmp(msg.channel, MIMI_CHAN_VOICE) == 0) ? 6 : MIMI_AGENT_MAX_HISTORY;
        session_get_history_json(msg.chat_id, history_json,
                                 MIMI_LLM_STREAM_BUF_SIZE, history_limit);

        cJSON *messages = cJSON_Parse(history_json);
        if (!messages) messages = cJSON_CreateArray();
        int base_message_count = cJSON_GetArraySize(messages);
        ...
        while (iteration < MIMI_AGENT_MAX_TOOL_ITER) {
            ...
            log_heap_state(iteration == 0 ? "agent_loop: before first llm_chat_tools" : "agent_loop: before followup llm_chat_tools");

            llm_response_t resp;
            err = llm_chat_tools(system_prompt, messages, tools_json, &resp);
            log_heap_state(iteration == 0 ? "agent_loop: after first llm_chat_tools" : "agent_loop: after followup llm_chat_tools");
            ...
            log_heap_state("agent_loop: before build_tool_results");
            cJSON *tool_results = build_tool_results(&resp, &msg, tool_output, TOOL_OUTPUT_SIZE);
            log_heap_state("agent_loop: after build_tool_results");
        ...
        /* Persist conversation history. For voice turns, skip intermediate tool_use/tool_result
         * blocks to keep later turns light on memory. */
        esp_err_t save_err = ESP_OK;
        bool persist_intermediate = strcmp(msg.channel, MIMI_CHAN_VOICE) != 0;
        int total_messages = cJSON_GetArraySize(messages);
        for (int i = base_message_count; i < total_messages; i++) {
            cJSON *persist_msg = cJSON_GetArrayItem(messages, i);
            cJSON *role = cJSON_GetObjectItem(persist_msg, "role");
            cJSON *content = cJSON_GetObjectItem(persist_msg, "content");
            bool is_intermediate_tool_msg = !persist_intermediate && role && cJSON_IsString(role) && content && cJSON_IsArray(content);
            if (is_intermediate_tool_msg) {
                continue;
            }
            save_err = session_append_message(msg.chat_id, persist_msg);
            if (save_err != ESP_OK) {
                break;
            }
        }
        ...
        if (final_text && final_text[0]) {
            ...
            esp_err_t save_final = ESP_ERR_NO_MEM;
            if (final_asst) {
                cJSON_AddStringToObject(final_asst, "role", "assistant");
                cJSON_AddStringToObject(final_asst, "content", final_text);
                save_final = session_append_message(msg.chat_id, final_asst);
                cJSON_Delete(final_asst);
            }

            if (save_err != ESP_OK || save_final != ESP_OK) {
                ESP_LOGW(TAG, "Session save failed for chat %s (history=%s, assistant=%s)",
                         msg.chat_id,
                         esp_err_to_name(save_err),
                         esp_err_to_name(save_final));
            } else {
