...
static struct {
    struct arg_str *path;
    struct arg_end *end;
} voice_chat_file_args;

static struct {
    struct arg_int *seconds;
    struct arg_end *end;
} voice_record_wav_args;

static int cmd_set_asr_provider(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&asr_provider_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, asr_provider_args.end, argv[0]);
        return 1;
    }
    asr_client_set_provider(asr_provider_args.provider->sval[0]);
    printf("ASR provider saved.\n");
    return 0;
}

static int cmd_set_asr_api_key(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&asr_api_key_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, asr_api_key_args.end, argv[0]);
        return 1;
    }
    asr_client_set_api_key(asr_api_key_args.key->sval[0]);
    printf("ASR API key saved.\n");
    return 0;
}

static int cmd_set_asr_model(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&asr_model_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, asr_model_args.end, argv[0]);
        return 1;
    }
    asr_client_set_model(asr_model_args.model->sval[0]);
    printf("ASR model saved.\n");
    return 0;
}

static int cmd_set_asr_base_url(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&asr_base_url_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, asr_base_url_args.end, argv[0]);
        return 1;
    }
    asr_client_set_base_url(asr_base_url_args.base_url->sval[0]);
    printf("ASR base URL saved.\n");
    return 0;
}

static int cmd_set_voice_base_url(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&voice_base_url_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, voice_base_url_args.end, argv[0]);
        return 1;
    }
    esp_err_t err = voice_bridge_set_base_url(voice_base_url_args.base_url->sval[0]);
    printf("voice_base_url status: %s\n", esp_err_to_name(err));
    return (err == ESP_OK) ? 0 : 1;
}

static int cmd_voice_send_file(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&voice_send_file_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, voice_send_file_args.end, argv[0]);
        return 1;
    }

    char text[1024];
    char audio_path[128];
    esp_err_t err = voice_bridge_send_file(voice_send_file_args.path->sval[0], text, sizeof(text), audio_path, sizeof(audio_path));
    printf("voice_send_file status: %s\n", esp_err_to_name(err));
    if (err == ESP_OK) {
        printf("text: %s\n", text);
        printf("audio: %s\n", audio_path);
    } else {
        printf("reason: %s\n", voice_bridge_get_unavailable_reason());
    }
    return (err == ESP_OK) ? 0 : 1;
}

static int cmd_voice_play(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&voice_play_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, voice_play_args.end, argv[0]);
        return 1;
    }

    esp_err_t err = voice_bridge_play_wav_file(voice_play_args.path->sval[0]);
    printf("voice_play status: %s\n", esp_err_to_name(err));
    if (err != ESP_OK) {
        printf("reason: %s\n", voice_bridge_get_unavailable_reason());
    }
    return (err == ESP_OK) ? 0 : 1;
...
static int cmd_voice_chat_file(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&voice_chat_file_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, voice_chat_file_args.end, argv[0]);
        return 1;
    }

    char text[1024];
    char audio_path[128];
    esp_err_t err = voice_bridge_send_and_play_file(voice_chat_file_args.path->sval[0], text, sizeof(text), audio_path, sizeof(audio_path));
    printf("voice_chat_file status: %s\n", esp_err_to_name(err));
    if (err == ESP_OK) {
        printf("text: %s\n", text);
        printf("audio: %s\n", audio_path);
    } else {
...
static int cmd_voice_record_wav(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&voice_record_wav_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, voice_record_wav_args.end, argv[0]);
        return 1;
    }

    int seconds = voice_record_wav_args.seconds->ival[0];
    if (seconds <= 0 || seconds > 30) {
        printf("voice_record_wav status: %s\n", esp_err_to_name(ESP_ERR_INVALID_ARG));
        printf("reason: seconds must be between 1 and 30\n");
        return 1;
    }

    const char *path = MIMI_SPIFFS_BASE "/voice_input.wav";
    esp_err_t err = voice_recorder_record_wav(path, (uint32_t)seconds * 1000U);
    printf("voice_record_wav status: %s\n", esp_err_to_name(err));
    if (err == ESP_OK) {
        printf("audio: %s\n", path);
    } else {
        printf("reason: %s\n", voice_recorder_get_unavailable_reason());
    }
    return (err == ESP_OK) ? 0 : 1;
}
...
static int cmd_config_show(int argc, char **argv)
{
    ...
    print_config("ASR Provider", MIMI_NVS_ASR,  MIMI_NVS_KEY_ASR_PROVIDER, MIMI_SECRET_ASR_PROVIDER, false);
    print_config("ASR Model",    MIMI_NVS_ASR,  MIMI_NVS_KEY_ASR_MODEL, MIMI_SECRET_ASR_MODEL, false);
    print_config("ASR Key",      MIMI_NVS_ASR,  MIMI_NVS_KEY_ASR_API_KEY, MIMI_SECRET_ASR_API_KEY, true);
    print_config("ASR URL",      MIMI_NVS_ASR,  MIMI_NVS_KEY_ASR_BASE_URL, MIMI_SECRET_ASR_BASE_URL, false);
...
esp_err_t serial_cli_init(void)
{
    ...
    /* set_asr_provider */
    asr_provider_args.provider = arg_str1(NULL, NULL, "<provider>", "ASR provider identifier");
    asr_provider_args.end = arg_end(1);
    esp_console_cmd_t asr_provider_cmd = {
        .command = "set_asr_provider",
        .help = "Set ASR provider identifier",
        .func = &cmd_set_asr_provider,
        .argtable = &asr_provider_args,
    };
    esp_console_cmd_register(&asr_provider_cmd);

    /* set_asr_api_key */
    asr_api_key_args.key = arg_str1(NULL, NULL, "<key>", "ASR API key");
    asr_api_key_args.end = arg_end(1);
    esp_console_cmd_t asr_api_key_cmd = {
        .command = "set_asr_api_key",
        .help = "Set ASR API key",
        .func = &cmd_set_asr_api_key,
        .argtable = &asr_api_key_args,
    };
    esp_console_cmd_register(&asr_api_key_cmd);

    /* set_asr_model */
    asr_model_args.model = arg_str1(NULL, NULL, "<model>", "ASR model identifier");
    asr_model_args.end = arg_end(1);
    esp_console_cmd_t asr_model_cmd = {
        .command = "set_asr_model",
        .help = "Set ASR model identifier",
        .func = &cmd_set_asr_model,
        .argtable = &asr_model_args,
    };
    esp_console_cmd_register(&asr_model_cmd);

    /* set_asr_base_url */
    asr_base_url_args.base_url = arg_str1(NULL, NULL, "<url>", "ASR base URL");
    asr_base_url_args.end = arg_end(1);
    esp_console_cmd_t asr_base_url_cmd = {
        .command = "set_asr_base_url",
        .help = "Set ASR base URL",
        .func = &cmd_set_asr_base_url,
        .argtable = &asr_base_url_args,
    };
    esp_console_cmd_register(&asr_base_url_cmd);

    /* set_voice_base_url */
    voice_base_url_args.base_url = arg_str1(NULL, NULL, "<base_url>", "Voice bridge base URL, e.g. http://192.168.1.10:8000");
    voice_base_url_args.end = arg_end(1);
    esp_console_cmd_t voice_base_url_cmd = {
        .command = "set_voice_base_url",
        .help = "Set voice bridge base URL",
        .func = &cmd_set_voice_base_url,
        .argtable = &voice_base_url_args,
    };
    esp_console_cmd_register(&voice_base_url_cmd);

    /* voice_send_file */
    voice_send_file_args.path = arg_str1(NULL, NULL, "<path>", "Send local wav/pcm file to /voice-chat");
    voice_send_file_args.end = arg_end(1);
    esp_console_cmd_t voice_send_file_cmd = {
        .command = "voice_send_file",
        .help = "Upload local audio file to voice bridge and download reply wav",
        .func = &cmd_voice_send_file,
        .argtable = &voice_send_file_args,
    };
    esp_console_cmd_register(&voice_send_file_cmd);

    /* voice_play */
    voice_play_args.path = arg_str1(NULL, NULL, "<path>", "Play a local 16k mono 16-bit wav file over I2S");
    voice_play_args.end = arg_end(1);
    esp_console_cmd_t voice_play_cmd = {
        .command = "voice_play",
        .help = "Play local wav file over MAX98357A via I2S",
        .func = &cmd_voice_play,
        .argtable = &voice_play_args,
    };
    esp_console_cmd_register(&voice_play_cmd);

    /* voice_chat_file */
    voice_chat_file_args.path = arg_str1(NULL, NULL, "<path>", "Upload local audio file to voice bridge and auto-play reply wav");
    voice_chat_file_args.end = arg_end(1);
    esp_console_cmd_t voice_chat_file_cmd = {
        .command = "voice_chat_file",
        .help = "Upload local audio file and automatically play the returned wav",
        .func = &cmd_voice_chat_file,
        .argtable = &voice_chat_file_args,
    };
    esp_console_cmd_register(&voice_chat_file_cmd);

    /* voice_record_wav */
    voice_record_wav_args.seconds = arg_int1(NULL, NULL, "<seconds>", "Record from INMP441 to /spiffs/voice_input.wav");
    voice_record_wav_args.end = arg_end(1);
    esp_console_cmd_t voice_record_wav_cmd = {
        .command = "voice_record_wav",
        .help = "Record local wav file from INMP441 microphone",
        .func = &cmd_voice_record_wav,
        .argtable = &voice_record_wav_args,
    };
    esp_console_cmd_register(&voice_record_wav_cmd);
