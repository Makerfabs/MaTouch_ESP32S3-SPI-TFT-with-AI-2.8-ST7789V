                                    ...
                                    const mp_obj_t *all_args)
{
    ...
    // The display manager initializes the shared SPI3 bus. Mount the SPI SD
    // device only after that bus exists, so the LCD and SD card can coexist.
    if (!esp_vision_storage_sdcard_is_mounted() &&
        esp_vision_storage_sdcard_try_mount() == ESP_OK) {
        esp_vision_storage_mount_sdcard_micropython();
    }
